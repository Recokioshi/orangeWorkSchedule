...
describe("renders calendar", () => {
        ...
        <main class=\\"makeStyles-content-2\\">
          ...
          <div class=\\"MuiFormControl-root makeStyles-calendarSelect-1\\"><label class=\\"MuiFormLabel-root MuiInputLabel-root MuiInputLabel-formControl MuiInputLabel-animated MuiInputLabel-shrink MuiFormLabel-filled\\" data-shrink=\\"true\\" id=\\"select-label-Month\\">Month</label>
            <div class=\\"MuiInputBase-root MuiInput-root MuiInput-underline MuiInputBase-formControl MuiInput-formControl\\">
              <div class=\\"MuiSelect-root MuiSelect-select MuiSelect-selectMenu MuiInputBase-input MuiInput-input\\" tabindex=\\"0\\" role=\\"button\\" aria-haspopup=\\"listbox\\" aria-labelledby=\\"select-label-Month select-Month\\" id=\\"select-Month\\">April</div><input aria-hidden=\\"true\\" tabindex=\\"-1\\" class=\\"MuiSelect-nativeInput\\" value=\\"3\\"><svg class=\\"MuiSvgIcon-root MuiSelect-icon\\" focusable=\\"false\\" viewBox=\\"0 0 24 24\\" aria-hidden=\\"true\\">
  ...
  test("select month", () => {
    ...
    const selectNameRegex = new RegExp(`Month ${months[currentMonth]}`);
    ...
    userEvent.click(within(getByRole("listbox")).getByText(new RegExp(newMonth)));
    expect(getByRole("button", { name: new RegExp(`Month ${newMonth}`)})).toBeInTheDocument();
  });
  ...
  test("select year", () => {
    ...
    userEvent.click(getByRole("button", { name: selectNameRegex }));
    userEvent.click(within(getByRole("listbox")).getByText(new RegExp(String(nextYear))));
    expect(getByRole("button", { name: new RegExp(`Year ${nextYear}`)})).toBeInTheDocument();
  });
});
